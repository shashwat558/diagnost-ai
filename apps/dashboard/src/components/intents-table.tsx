    ...
    <>
      <div className="flex items-center gap-2 px-6">
        <div className="relative flex-1">
          <Icon name="search" className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-ink-subtle" />
          <Input
            value={q}
        ...
        <table className="w-full border-separate border-spacing-0 text-sm">
          <thead>
            <tr className="text-left text-sm text-ink-muted">
              <th className="w-8 border-b border-line py-2" title="Select for bulk actions">●</th>
              ...
              <th className="border-b border-line py-2 pr-4 text-right font-normal"># Conversations</th>
              <th className="border-b border-line py-2 pr-4 font-normal">
                Trend
          ...
          <tbody>
            {filtered.map((r) => {
              ...
              return (
                <tr key={r.id} className="group hover:bg-hover">
                  <td className="border-b border-line py-2.5 align-middle">
                    <span className="ml-1 inline-block h-3.5 w-3.5 rounded-full border border-line-strong" />
                  </td>
                  <td className="border-b border-line py-2.5 pr-4">
                    <Link
                      href={`/clusters/${r.id}`}
                      className="font-medium text-ink underline decoration-line underline-offset-2 hover:decoration-ink-muted"
                    >
                    ...
                    <span className="mt-0.5 block max-w-lg truncate text-sm text-ink-muted" title={r.summary}>{r.summary}</span>
                  </td>
                  <td className="border-b border-line py-2.5 pr-4 text-right tabular-nums text-ink">
                    {Number(r.size)}
                  </td>
                  <td className="border-b border-line py-2.5 pr-4">
                    <Sparkline
                  ...
                  <td className="border-b border-line py-2.5 pr-4">
                    {delta === null ? (
                      <span className="text-ink-subtle" title="Not enough history yet">—</span>
                    ) : (
                      <span
                        title={delta > 0 ? "Growing vs yesterday" : delta < 0 ? "Shrinking vs yesterday" : "Flat vs yesterday"}
                        className={delta > 0 ? "text-emerald-600" : "text-ink-muted"}
                      >
                        {delta > 0 ? "+" : ""}
                        {delta}%
                      </span>
                  ...
                  <td className="border-b border-line py-2.5 pr-4 tabular-nums text-ink">
                    {(r.error_rate * 100).toFixed(0)}%
                  </td>
                  <td className="border-b border-line py-2.5 text-ink-muted">{relTime(r.lastSeen)}</td>
                </tr>
              );
            })}
            {filtered.length === 0 && (
              <tr>
                <td colSpan={7} className="py-10 text-center text-ink-subtle">
                  No intents match “{q}”.
                </td>
              </tr>
            )}
