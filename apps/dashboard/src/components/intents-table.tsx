    ...
    <>
      <div className="flex items-center gap-2 px-6">
        <div className="relative flex-1">
          <Icon name="search" className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-gray-400 dark:text-gray-500" />
          <Input
            value={q}
      ...
      {notice && (
        <div className="mx-6 mt-3 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700">
          {notice}
        </div>
      )}
        ...
        <table className="w-full border-separate border-spacing-0 text-sm">
          <thead>
            <tr className="text-left text-sm text-gray-500 dark:text-gray-400">
              <th className="w-8 border-b border-gray-200 dark:border-gray-800 py-2" title="Select for bulk actions">●</th>
              <th className="border-b border-gray-200 dark:border-gray-800 py-2 pr-4 font-normal">
                User intent
                <HelpTip text="A group of similar conversations, clustered automatically. Click one to see examples and failures." />
              </th>
              <th className="border-b border-gray-200 dark:border-gray-800 py-2 pr-4 text-right font-normal"># Conversations</th>
              <th className="border-b border-gray-200 dark:border-gray-800 py-2 pr-4 font-normal">
                Trend
          ...
          <tbody>
            {filtered.map((r) => {
              ...
              return (
                <tr key={r.id} className="group hover:bg-gray-50 dark:hover:bg-gray-800/60">
                  <td className="border-b border-gray-100 dark:border-gray-800 py-2.5 align-middle">
                  ...
                  <td className="border-b border-gray-100 dark:border-gray-800 py-2.5 pr-4">
                    <Link
                      href={`/clusters/${r.id}`}
                      className="font-medium text-gray-900 dark:text-gray-100 underline decoration-gray-300 underline-offset-2 hover:decoration-gray-500"
                    >
                      {r.intent.replace(/_/g, " ")}
                    </Link>
                    {needsInstruction && (
                      <span
                        className="ml-2 rounded bg-accent-soft px-1.5 py-0.5 text-xs font-medium text-accent"
                    ...
                    <span className="mt-0.5 block max-w-lg truncate text-sm text-gray-500 dark:text-gray-400" title={r.summary}>{r.summary}</span>
                  </td>
                  ...
                  <td className="border-b border-gray-100 dark:border-gray-800 py-2.5 pr-4">
                    {delta === null ? (
                      <span className="text-gray-400 dark:text-gray-500" title="Not enough history yet">—</span>
                    ) : (
                      <span
                        title={delta > 0 ? "Growing vs yesterday" : delta < 0 ? "Shrinking vs yesterday" : "Flat vs yesterday"}
                        className={delta > 0 ? "text-emerald-600" : "text-gray-500 dark:text-gray-400"}
                      >
                        {delta > 0 ? "+" : ""}
                        {delta}%
                      </span>
                  ...
                  <td className="border-b border-gray-100 dark:border-gray-800 py-2.5 pr-4 tabular-nums text-gray-700 dark:text-gray-300">
                    {(r.error_rate * 100).toFixed(0)}%
                  </td>
                  <td className="border-b border-gray-100 dark:border-gray-800 py-2.5 text-gray-500 dark:text-gray-400">{relTime(r.lastSeen)}</td>
                </tr>
              );
            })}
            {filtered.length === 0 && (
