...
export function MetricsSection() {
  return (
    <div className="space-y-6">
      ...
      <Reveal delay={100}>
        <div className="grid border border-line md:grid-cols-4 bg-surface">
          {METRICS.map((m, i) => (
            <div
              key={m.label}
              className={`group relative p-8 transition-all duration-300 hover:bg-hover ${
                i > 0 ? "border-line max-md:border-t md:border-l" : ""
              }`}
            >
              <div className="font-tech text-xs uppercase tracking-wider text-brand mb-2 font-semibold">
                {m.label}
              </div>

              <div className="font-tech text-4xl md:text-5xl font-medium leading-none tracking-[-0.06em] text-ink group-hover:text-brand transition-colors">
                <CountUp end={m.value} decimals={m.decimals} />
                {m.unit && <span className="ml-1 text-2xl text-ink-muted">{m.unit}</span>}
              </div>

              <div className="mt-3 font-tech text-xs leading-5 text-ink-muted group-hover:text-ink-muted transition-colors">
                {m.sub}
              </div>
